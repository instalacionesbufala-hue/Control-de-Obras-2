...
export const KitsView: React.FC<Props> = ({ kits, catalogItems, catalogCategories, onSaveKit, onDeleteKit, onDuplicateKit, onUsarEnPresupuesto, companySettings, embebido = false }) => {
  ...
  return (
    <div className={embebido ? "space-y-6" : "p-6 md:p-8 space-y-6 max-w-7xl mx-auto"}>
      <div className={embebido ? "hidden" : "flex flex-col sm:flex-row sm:items-center justify-between gap-4"}>
        <div>
      ...
      {!embebido && <div className="p-4 bg-blue-50/70 border border-blue-100 rounded-2xl text-xs text-blue-900 flex items-start gap-2">
